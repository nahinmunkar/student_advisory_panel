...
const Auth = ({ location }) => {
  ...
  // using useEffect to prevent user from coming to auth through URL directly
  useEffect(() => {
  ...
  // Toggle between sign-in and sign-up and reset fields
  const handleToggle = () => {
...
*/
  ...
  // Updated handleSubmit function to handle both sign-in and sign-up
    ...
    try {
      if (toggleLogin) {
        ...
        if (location.state === "Mentor") {
          ...
          // Sign up mentor (assuming mentorSignUp handles navigation or shows a success message)
        ...
        // Add sign-up logic for Admin here if needed.
      } else {
        // SIGN-IN LOGIC
        if (location.state === "Admin") {
          // Dispatch admin sign in and wait for the result.
          await dispatch(adminSignIn(fields, history));
          // Redirect to the admin dashboard.
          history.push("/admin/dashboard"); // Adjust route as needed.
        } else if (location.state === "Mentor") {
          await dispatch(mentorSignIn(fields));
          history.push("/mentor/dashboard"); // Adjust route as needed.
        } else if (location.state === "Mentee") {
          await dispatch(studentSignIn(fields));
          history.push("/mentee/dashboard"); // Adjust route as needed.
        }
      }
    } catch (error) {
      // In case any error occurs, show an error toast
      showToast("error", "Sign in failed", 5000, toast.POSITION.TOP_RIGHT);
  ...
  // State for the reCAPTCHA (only used during sign-up)
  //const [recaptcha, setRecaptcha] = useState(true);

  // Function to handle reCAPTCHA and send token to backend (only active on sign-up)
  /*
  const handleCaptchaChange = async (token) => {
    if (toggleLogin) {
      const isVerified = await dispatch(verifyRecaptcha(token));
      // Disable button if verification fails (you can adjust this logic as needed)
      setRecaptcha(!isVerified);
    }
  };
  */
      ...
      <div className="flex-2 bg-gray-600 h-full flex items-center justify-center">
        <div className="w-96">
          <form onSubmit={handleSubmit}>
            ...
            {/* Render reCAPTCHA only for sign-up */}
